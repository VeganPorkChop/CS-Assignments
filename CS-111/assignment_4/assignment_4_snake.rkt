...
(define (drop-tail lst)
  (cond[(=(length lst)1) '()]
       [else (append (list(first lst)) (drop-tail (rest lst)))]))
...
(define (delete-posn lst p)
  (if (equal? (first lst) p) (rest lst) (cons (first lst) (delete-posn (rest lst) p))))

(check-expect (delete-posn (list (make-posn 1 2) (make-posn 3 4) (make-posn 5 6)) (make-posn 3 4)) (list (make-posn 1 2) (make-posn 5 6)))

;; contains-posn? : (listof posn) posn -> boolean
(define (contains-posn? lst p)
  (not(equal? (delete-posn lst p) lst)))
