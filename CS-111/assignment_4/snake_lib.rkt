...
(define-global-free-variables
  make-game game-snake game-obstacles game-foods game-ticks
  snake-direction snake-body)
...
(define (make-body-image color)
  (square (- cell-length 3) "solid" color))
...
(define/implicit-parameter (draw-game g)
  #:freevars ([game-over? default-game-over?]
              game-score)
  (define over? (game-over? g))
  ...
  (place-items
   ...
   (place-items
    (if over? dead-body alive-body)
    (cdr (snake-body (game-snake g)))
    (place-items obstacle-image
                 (game-obstacles g)
...
(define/implicit-parameter (play-game initial-game)
  ...
  (local [;; copy-game/increment-tick : game -> game
          ...
          ;; game-tick : game -> game
          (define (game-tick g)
            (local [(define morsels (length (game-foods g)))
                    (define f
                      (copy-game/increment-tick
                       (next-game-state g)))]
              (cond [(= (length (game-foods f)) morsels) f]
                    [else (copy-game/new-food (random-free-posn f) f)])))

          ;; game-key : game key-event -> game
          (define (game-key g e)
            (cond [(member e '("up" "down" "left" "right"))
                   (copy-game/new-snake-direction e g)]
                  [else g]))]
...
(define-syntax (game-launcher stx)
  (datum->syntax
   ...
   '(racket:module+
     test
     (racket:require racket/base racket/class embedded-gui)
